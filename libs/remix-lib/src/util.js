...
export function toHexPaddedString(v) {
    if (v) {
        if (typeof v === 'string') {
...
*/
export function findClosestIndex(target, array) {
    if (array.length === 0) {
        return -1;
    }
    const index = findLowerBound(target, array);
...
export function sha3_256(value) {
    ...
    const retInBuffer = hash.keccak(setLengthLeft(value, 32));
...
export function extractinputParameters(value) {
    ...
    if (inputsParam)
        value = value.replace(inputsParam, '');
    return value;
}
...
export function compareByteCode(code1, code2) {
    ...
    if (code1 === '0x00' || code2 === '0x00' && code1 !== code2)
        return false; // // This can be removed some time once YUL returns correct bytecode
