...
export class TxListener {
    constructor(opt, executionContext) {
        ...
        opt.event.udapp.register('callExecuted', (error, from, to, data, lookupOnly, txResult) => __awaiter(this, void 0, void 0, function* () {
            if (error)
                return;
            ...
            if (this.executionContext.isVM()) {
                execResult = yield this.executionContext.web3().remix.getExecutionResultFromSimulator(txResult.transactionHash);
        ...
        opt.event.udapp.register('transactionExecuted', (error, from, to, data, lookupOnly, txResult) => {
            ...
            if (this._loopId)
                return; // we seems to already listen on a "web3" network
            this.executionContext.web3().eth.getTransaction(txResult.transactionHash).then((tx) => __awaiter(this, void 0, void 0, function* () {
                let execResult;
                if (this.executionContext.isVM()) {
                    execResult = yield this.executionContext.web3().remix.getExecutionResultFromSimulator(txResult.transactionHash);
                }
